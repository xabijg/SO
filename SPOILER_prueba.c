...
//       } else {
//
//         for (size_t s = 0; s < j-1; s++) {
//
//           listFileWithVariables( flags, files[s] );
//
//         }
//
//       }
...
void listDirWithVariables( char * flags , char * argDir ) {
  ...
  if (flags[3] == 'r') { rec = true; }
    ...
    if (lon==true) {
      ...
      if (rec==true){ listDirRecLong(path, hid); }
