...
int statsUpdate(listElement *item, int signal) {
...
int freeE(void *elem);
int Cmd_listarprocs (char *trozos[], int ntrozos,list l);
int Cmd_deleteProcess (char * trozos[], int ntrozos,list l);
int Cmd_proc (char *trozos[], int ntrozos,list l);
    ...
    while (!terminado) {
        ...
        if ( trozos[0] == NULL ) {
            ...
            if ( strcmp(trozos[0], "getpriority") == 0 ) {
                ...
                Cmd_listarprocs (trozos,ntrozos,l);
                ...
                Cmd_deleteProcess(trozos,ntrozos,l);

            }else if ( strcmp(trozos[0], "proc") == 0 ) {

                Cmd_proc(trozos,ntrozos,l);
...
int search(list *l,listElement **p,pid_t pid) {
    *p = NULL;
    for (pos i = first(*l); !last(i); i = nextE(i)) {
        listElement *item = getElement(i);
...
int Cmd_listarprocs (char *trozos[], int ntrozos,list i){ 
    for(pos j=first(i);!last(j);j=nextE(j))
        showE(getElement(j));
    return 0;
}

int Cmd_proc (char *trozos[], int ntrozos,list i){
    switch (ntrozos){
    case 1: 
...
int Cmd_deleteProcess (char * trozos[], int ntrozos,list i){
